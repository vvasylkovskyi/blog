...
export const getBlogsData = async (): Promise<any[]> => {
  try {
    const baseDirectoryPathForBlogPostsJsonData = path.join(
      process.cwd(),
      "../blog-content/data"
    );

    const baseDirectoryPathForBlogText = path.join(
      process.cwd(),
      "../blog-content/ready"
    );
    ...
    const arrayOfBlogPostsData: Array<any> = await Promise.all(
      files.map((filePath: string) =>
        JSON.parse(
          fs.readFileSync(
            `${baseDirectoryPathForBlogPostsJsonData}/${filePath}`,
            "utf8"
          )
        )
      )
